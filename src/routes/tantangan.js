  ...
  try {
    ...
    await pool.query(
      `INSERT INTO solutions (challenge_id, nama_peneliti, fakultas, ringkasan, ki_terkait)
       VALUES ($1,$2,$3,$4,$5)`,
      [values.challenge_id, values.nama_peneliti, values.fakultas, values.ringkasan, values.ki_terkait]
    );
...
router.post('/pasang-kebutuhan', async (req, res, next) => {
  try {
    ...
    const maxRes = await pool.query('SELECT COALESCE(MAX(id), 0)::int AS n FROM challenges');
    const kode = `TI-${new Date().getFullYear()}-${String(maxRes.rows[0].n + 1).padStart(4, '0')}`;
    await pool.query(
      ...
      [kode, values.judul, values.perusahaan, values.kontak, values.bidang, values.deskripsi, values.kebutuhan_spesifik, values.skema, values.deadline]
    );
